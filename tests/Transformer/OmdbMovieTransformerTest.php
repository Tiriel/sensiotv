...
class OmdbMovieTransformerTest extends TestCase
{
    ...
    public function testGenreAreTakenFromDatabaseWhenTheyExistWithProphecy()
    {
        $this->markTestSkipped('To be reworked');
        $data = [
            'Title' => 'Star Wars: Episode IV - A New Hope',
        ...
        /** @var ObjectProphecy|Genre $genre */
        $genre = $this->prophesize(Genre::class);
        $genre->getId()->willReturn(1);
        $genre->getName()->willReturn('Action');

        /** @var ObjectProphecy|GenreRepository $repository */
        $repository = $this->prophesize(GenreRepository::class);
        $repository->findOneBy(Argument::any())->shouldBeCalledTimes(3);
        $repository->findOneBy(Argument::any())->will(function () use ($genre) {
                $args = func_get_args();
        ...
        $transformer = new OmdbMovieTransformer($repository->reveal());
